...
if( isset( $_POST['voucher'] )  )
{
    ...
    $dadosReserva = $pdo->prepare(
        "SELECT r.idservico,r.id,pax, documento, dateinput, dateoutput, photoresident, c.fullname as cliente, s.fullname as `status`, r.horaap, u.firstname, u.lastname,
                  ...
                  pricechild,numbervoucher, r.valueservice, r.abertura, se.screenplay, c.observacao
    ...
    $adicionais = $pdo->prepare(
        'SELECT ra.idservice,ra.dateinput as ap, s.fullname,s.screenplay, s.priceadult, s.pricechild, ss.schedule, qpax, qchild, qfree, ra.valueservice, ra.horaap,ra.documento
    ...
    <div align="center" class="container">
        <div class="col-md-12">
            ...
            <table class="responsive-table" style="font-size: 10px;">
                ...
                <?php if($contador > 0) { ?>
                    <?php foreach ($registro as $item){ $timestampAdd = strtotime( $item->ap );?>
                        <tr>
                            <tbody>
        ...
        <?php echo("<p style='font-size: 9px; font-weight: bold;'>".utf8_decode( strtoupper("SERVIÇO: ".$dadosGerais['serivco']))."</p>
                        <p style='font-size: 8px; text-align: justify;'>".strtoupper(utf8_encode($dadosGerais['screenplay']))."</p>"); ?>
        <?php if( $contador > 0 ){ ?>
            <?php foreach ( $registro as $item ){ ?>
                <?php echo("<p style='font-size: 9px; font-weight: bold;'>".utf8_decode( strtoupper("SERVIÇO: ".$item->fullname))."</p>
                        <p style='font-size: 8px; text-align: justify;'>".strtoupper( utf8_encode($item->screenplay))."</p>"); ?>
            <?php }?>
        <?php }?>
