...
{
    $new_order_driver = $pdo->prepare('insert into `ct_orderservice` values (DEFAULT, :namedriver, :voucher, :phone, :tpax, :tchild, :tfree ,:idservico, :apanha, :doc, :namepax, :datee) ');
...
if( isset($_POST['buscamotorista']) )
{
    $find_driver = $pdo->prepare('SELECT `namedriver` as motorista FROM `ct_orderservice` os where os.`date` >= :inicio and os.`date` <= :fim GROUP by `namedriver`');
    $find_driver->execute( array(":inicio" => $_POST['inicio'], ":fim" => $_POST['fim']));
...
<div class="page-content--bgf7">
    ...
    <div class="">
        ...
        <div class="">
                                ...
                                <div class="table-responsive">
                                    <table id="myTable" class="table table-striped table-bordered dataTable">
                                        ...
                                        <?php for( $i= 0; $i <= count( $servico ); $i++ ){
                                            ...
                                            ?>
                                            <?php foreach ($dadosGerais as $item) {$totalPax1 = $totalPax1 + $item->qtdpax; $totalChild1 = $totalChild1 + $item->qtdchild; $totalfree1 += $item->qtdfree;?>
                                                <tr>
                                                    ...
                                                    <td style="cursor: pointer;" onclick="orderService('<?php echo($item->numbervoucher); ?>','<?php echo($item->pax); ?>','<?php echo($item->photoresident); ?>', '<?php echo($item->qtdpax); ?>',
                                                        ...
                                                        '<?php echo($item->documento); ?>', this, '<?php echo(date("Y/m/d")); ?>', '<?php echo($item->qtdfree); ?>')">Incluir Serviço</td>
                                                </tr>
                                            <?php } ?>
                                            <?php if( $contador > 0 ){ ?>
                                                <?php foreach ($registro as $item2) {$totalPax2 = $totalPax2 + $item2->qpax; $totalchil2 = $totalchil2 + $item2->qchild; $totalfree2 += $item2->qfree; ?>
                                                    <tr>
                                                        ...
                                                        <td style="cursor: pointer;" onclick="orderService('<?php echo($item2->numbervoucher); ?>','<?php echo($item2->pax); ?>','<?php echo($item2->photoresident); ?>', '<?php echo($item2->qpax); ?>',
                                                            ...
                                                            '<?php echo($item2->documento); ?>', this, '<?php echo(date("Y/m/d")); ?>', '<?php echo($item2->qfree); ?>')">Incluir Serviço</td>
                                                    </tr>
                                                <?php } ?>
                                            <?php }?>
            ...
            <div class="card">
                ...
                <div id="collapseTree" class="collapse show" aria-labelledby="headingTree" data-parent="#accordionExample">
                    <div class="card-body">
                        ...
                        <form method="post">
                            <div class="col-md-6 pull-left">
                                <label for="inicio">Data de cadastro Inicial</label>
                                <input required class="form-control" name="inicio" id="inicio" type="date">
                            </div>
                            <div class="col-md-6 pull-right">
                                <label for="fim">Data de cadastro Final</label>
                                <input required class="form-control" name="fim" id="fim" type="date">
                            </div>
                            <div class="container-fluid">
                                <button type="submit" name="buscamotorista" class="btn btn-success btn-lg">Buscar Motoristas</button>
                            </div>
                        </form>
                        <hr>
                    </div>
            ...
            <?php if( isset( $_POST['buscamotorista'] ) ){ ?>
                <div class="modal" tabindex="-1" role="dialog">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">Motoristas do dia:  <?php echo( date("d/m/Y", strtotime($_POST['inicio'] ) )." até ".date("d/m/Y", strtotime($_POST['fim'] ) ) ); ?></h5>
                            ...
                            <div class="modal-body">
                                <form autocomplete="on" action="relatorio/print-motorista.php" method="post" target="_blank">
                                    <div class="form-group">
                                        <label for="inicio">Motorista</label>
                                        <select class="form-control" name="motorista">
                                            <?php foreach ($data_find_driver as $item){ ?>
                                                <option value="<?php echo( $item->motorista ); ?>"><?php echo( $item->motorista ); ?></option>
                                            <?php }?>
                                        </select>
                                    </div>
                                    <input  name="inicio" id="inicio" type="hidden" value="<?php echo( $_POST['inicio'] ); ?>">
                                    <input  name="fim" id="fim" type="hidden" value="<?php echo( $_POST['fim'] ); ?>">
                                    <div class="container-fluid">
                                        <button type="submit"  class="btn btn-success btn-lg btn-block">Imprimir</button>
                                    </div>
                                </form>
                            </div>
                        </div>
        ...
        <script type="text/javascript">
            function orderService(voucher, namepax, phone, tpax, tchild, idservico, apanha, doc, linha,  checkin, qfree){
                ...
                if( nomemotorista == '' )
                {
                    ...
                    $.ajax({
                        ...
                        data: {namedriver: nomemotorista, voucher: voucher, phone:phone, tpax:tpax, tchild:tchild, idservico:idservico, apanha:apanha, doc:doc, namepax:namepax, datee:checkin, qfree: qfree },
                        dataType: 'json',
                ...
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        $.each(response,  function (key, item) {
                            $("#motorista").append(`<option value="${item.motorista}">${item.motorista}</option>`);
                        });
