    ...
    <div class="container">
            ...
            <?php if( $idservicos[0] == 0 ) {
                ...
                <?php foreach( $registro as $item ) {
                    $financeiroReserva = $pdo->prepare(
                        'SELECT tarifa, valuecredit as credito, valueguia as guia, valueagente AS agente
                              FROM `ct_createfaturacredit` where numbervoucher = :voucher ');
                    $financeiroReserva->execute(array(":voucher" =>  $item->numbervoucher));
                    $dadosFinanceiro_l = $financeiroReserva->fetchAll( PDO::FETCH_CLASS );
                    $dadosFinanceiro = array();
                    foreach($dadosFinanceiro_l as $financeiro)
                    {
                        $dadosFinanceiro['guia'] += $financeiro->guia; 
                        $dadosFinanceiro['agente'] += $financeiro->agente; 
                        $dadosFinanceiro['credito'] += $financeiro->credito; 
                    }    
                    ...
                    $totalRecebido = $totalRecebido + $dadosFinanceiro['credito'];
                    ...
                    <?php if( $contadorDescricaoPagamento >0 ){ ?>
                        <?php foreach ($registroDescricao as $item3){ ?>
                            <tr>
                                ...
                                <td id="desc" colspan="2"><?php echo("<strong>Metodo</strong> ".$item3->pagamento); ?></td>
        ...
        <table class="highlight">
            ...
            <tr>
                ...
                <td><?php echo("R$ ".number_format( $totalRecebido, 2, ",", "." ) ) ?></td>
                ...
                <?php foreach( $registro as $item ) {
                    ...
                    <?php if( $contadorDescricaoPagamento >0 ){ ?>
                        <?php foreach ($registroDescricao as $item3){ ?>
                            <?php if( $item3->valor > 0 ){ ?>
                                <tr>
                                    <td id="desc" colspan="2"><?php echo("<strong>Data Pagamento</strong> ".date("d-m-Y", strtotime($item3->dia))); ?></td>
                                    <td id="desc" colspan="2"><?php echo("<strong>Metodo</strong> ".$item3->pagamento); ?></td>
                                    <td id="desc" colspan="2">
                                        <?php echo("<strong>Valor R$ </strong>".number_format($item3->valor, 2,",", ".")); ?>
                                    </td>
